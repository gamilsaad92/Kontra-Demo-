...
function rid(prefix:string='id'){ return prefix + '_' + Math.random().toString(36).slice(2,10) }
...
function reducer(state: State, action: Action): State {
  switch(action.type){
    ...
    case 'SET_ORG': return { ...state, orgId: action.orgId }
    case 'CREATE_APP': {
      const app: Application = { id: rid('app'), status:'Submitted', createdAt:new Date().toISOString(), docs:[], ...action.app }
      return { ...state, applications: [app, ...state.applications] }
    }
    case 'ADD_DOC': {
      return { ...state, applications: state.applications.map(a=> a.id===action.id ? { ...a, docs: [...a.docs, action.doc] } : a) }
    }
    case 'SET_RISK': {
      return { ...state, applications: state.applications.map(a=> a.id===action.id ? { ...a, riskScore: action.score } : a) }
    }
    case 'DECIDE_APP': {
      return { ...state, applications: state.applications.map(a=> a.id===action.id ? { ...a, status: action.status } : a) }
    }
    case 'CREATE_ESCROW': {
    ...
    case 'MAKE_PAYMENT': {
      return { ...state, payments: state.payments.map(p=> p.id===action.paymentId ? { ...p, status:'Paid' } : p) }
    }
    case 'LIST_TRADE': {
...
export function DemoProvider({children}:{children:React.ReactNode}){
  ...
  useEffect(()=>{ localStorage.setItem('kontra_demo_state', JSON.stringify(state)) }, [state])
